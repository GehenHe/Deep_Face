...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, payload, isBinary):
        ...
        if msg['type'] == "ALL_STATE":  #load all state training equal false
            ...
            self.Load_Id("/home/msff/deepface_super_3/demos/web/ID")
            self.trainKNN()
            ...
            print "temp_save:"
            #elf.Load_Id("/home/msff/deepface_super_3/demos/web/ID")
            #self.trainKNN()
            print self.people
    ...
    def Load_Id(self,ID_path):
        im_dir = glob(ID_path+'/*')
        for index in range(len(im_dir)):
            img_name = im_dir[index].split('/')[-1]
            print "name is {}".format(img_name)
            print "id is {}".format(index)
            ...
            self.sendMessage(json.dumps(msg))
            for img_dir in glob(im_dir[index]+'/*.jpg'):
                try:
                    self.Add_ID(img_dir,index)
                except:
                    pass
